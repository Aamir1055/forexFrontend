          ...
          <thead className={`border-b transition-colors ${
            ...
            <tr>
              ...
              <th 
                ...
                className="px-2 py-1.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wide cursor-pointer hover:bg-slate-50 transition-colors"
                title="Click to sort"
              >
                <div className="flex items-center space-x-1">
                  <span>User</span>
              ...
              <th className="px-2 py-1.5 text-left text-xs font-medium text-slate-600 uppercase tracking-wide">
                Changes
              </th>
            </tr>
          </thead>
            ...
            {logs.map((log, index) => (
              ...
              >
                <td className="px-2 py-2">
                  <p className="text-xs font-semibold text-slate-900">{log.id}</p>
                </td>
                <td className="px-2 py-2">
                  <div>
                ...
                <td className="px-2 py-2">
                  <p className="text-xs text-slate-700">{log.table_name}</p>
                </td>
                <td className="px-2 py-2">
                  <p className="text-xs text-slate-700">{log.record_id}</p>
                </td>
                <td className="px-2 py-2">
                  <p className="text-xs text-slate-700">{log.ip_address}</p>
                </td>
                <td className="px-2 py-2">
                  <p className="text-xs text-slate-700">{formatDate(log.created_at)}</p>
                </td>
                <td className="px-2 py-2">
                  <div className="max-w-xs overflow-hidden">
                    {log.new_values && (
                      <div className="mb-0.5">
                        ...
                        <span className="text-xs text-slate-600 truncate">{formatValues(log.new_values)}</span>
                      </div>
                    )}
                    {log.old_values && (
                      <div>
                        <span className="text-xs font-semibold text-red-600">Old: </span>
                        <span className="text-xs text-slate-600 truncate">{formatValues(log.old_values)}</span>
                      </div>
                    )}
                  </div>
                </td>
